import CollectionCard from '@/components/common/collection-launchpad-card'
...
const SectionLaunchPad = () => {
    ...
    useEffect(() => {
        ;(async () => {
            try {
                const res = await collectionServices.getLauchPadCollections({
                    page: 1,
                    limit: 5,
                })
    ...
    return (
        <>
            {!!collections.length && (
                <Container fluid>
                    <TitleHeader title={t('LAUNCH_PAD')} link="/" />
