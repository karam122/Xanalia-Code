...
export const getNFTImage = (smallImage: string, previewImage: string) => {
    if (previewImage) {
        return previewImage
    }
    if (smallImage) {
    ...
    return '/images/default-nft-image.jpeg'
}
...
const AllActivity = () => {
    ...
    const renderItem = (item: any, index: number) => {
        ...
        return (
            <Link
                key={index}
                href={`/assets/${item?.nft?.network?.name}/${item?.nft?.collections?.contractAddress}/${item?.nft?.tokenId}`}
                passHref
            >
                <tr
                    key={index}
